...
parser = argparse.ArgumentParser(
    description='Version 0.1. This script assists to prepare a newick tree for mcmctree in the PAML package. Use pipe to introduce multiple bounds sequentially.')
parser.add_argument('--left_species', metavar='STR', default=None, type=str, required=True,
                    help='default=%(default): Any species in the left clade. If you want to set a bound on the node splitting Homo_sapiens and Mus_musculus, specify one of them (e.g., Homo_sapiens).')
parser.add_argument('--right_species', metavar='STR', default=None, type=str, required=True,
...
parser.add_argument('--tree', metavar='PATH', default=None, type=str, required=True,
                    help='default=%(default): newick tree file path. "-" for standard input.')
parser.add_argument('--add_header', action='store_true',
                    help='default=%(default): Add the header required for mcmctree.')
